...
(function () {
  ...
  /**
   * state = {
   *   resources:   { [resourceName]: number },         // preço unitário
   *   sales:       { [craftId]: { npc, market } },     // preço unit. de venda
   *   tax:         number                              // taxa do market em %
   * }
   */
   ...
   * quantidade global). Poções sempre rendem 100 por receita; demais itens
   * comuns rendem 1. Berries usam lógica própria (média/sorte).
   */
  function getUnitsPerRecipe(craft) {
    ...
    if (craft.tipo === POTION_TIPO) return POTION_PER_RECIPE;
    return 1;
  }
  ...
  function calculateCraft(craft) {
    ...
    const taxFactor = 1 - (state.tax || 0) / 100;
  ...
  function init() {
    ...
    taxInput.addEventListener("input", () => {
      state.tax = parseNumberInput(taxInput.value);
      saveState();
      recalcAll();
    });
    ...
    document.getElementById("btnReset").addEventListener("click", () => {
      const ok = confirm(
        "Tem certeza que deseja apagar TODOS os preços salvos?"
      );
      if (!ok) return;
