/* ============================================================
   PXG Engineer Craft Calculator — App Core
   Persistência no localStorage, renderização dinâmica
   e motor de cálculo de custo/receita/lucro.

   Estrutura espelhada no adventurer.js, exceto:
   - sem sistema de berries (igual Aventureiro)
   - sub-receitas profundas (Iron Bar, Engine, Locker, Mecha parts)
   ============================================================ */
...
(function () {
  ...
  // ---------- Constantes ----------
  const LS_KEY = "pxg-engineer-craft-v1";
  ...
  function recomputeDepsAndRequirements() {
    ...
    if (safety <= 0) {
      console.warn("recomputeDepsAndRequirements: safety hit — possível ciclo em CRAFTS_ENGINEER");
    }
  }
  ...
  function buildResourceBadges(r) {
    ...
    if (subCraftId && subCraftId !== r.id) {
      ...
      b.textContent = "↳ Receita do Engenheiro";
      b.title = "Esse recurso também é uma receita do Engenheiro — o custo dele pode ser calculado a partir dos ingredientes dele";
      badges.push(b);
    }
    if (lootItem) {
  ...
  function recalcAll() {
    for (const craft of CRAFTS_ENGINEER) recalcCraft(craft.id);
  }
  ...
  // ---------- Inicialização ----------
  async function init() {
    if (typeof CRAFTS_ENGINEER === "undefined" || !Array.isArray(CRAFTS_ENGINEER)) {
      console.error("CRAFTS_ENGINEER não está disponível. Verifique engineer-data.js.");
      return;
    }
    ...
    if (typeof WikiOverrides !== "undefined") {
      ...
      WikiOverrides.applyToCrafts(CRAFTS_ENGINEER);
    ...
    for (const c of CRAFTS_ENGINEER) {
      craftsById[c.id] = c;
      craftIdByName.set(normalizeName(c.nome), c.id);
    }
    Object.assign(resourceIndex, buildResourceIndex(CRAFTS_ENGINEER));
    ...
    document.getElementById("craftCount").textContent =
      `${CRAFTS_ENGINEER.length} crafts`;

    const resources = getUniqueResources(CRAFTS_ENGINEER);
    renderResourceInputs(resources);

    const tipoSelect = document.getElementById("filterTipo");
    for (const t of getUniqueTipos(CRAFTS_ENGINEER)) {
    ...
    renderCraftCards(CRAFTS_ENGINEER);
