/* ============================================================
   PXG Stylist Craft Calculator — App Core
   Persistência no localStorage, renderização dinâmica
   e motor de cálculo de custo/receita/lucro.

   Estrutura espelhada no engineer.js, exceto:
   - DEFAULT_SEM_DROPPERS: Style Point, Fashion Point e variantes
     Refined são pré-marcados como matéria-prima na primeira carga
     (vêm da mecânica Drawing Book, não dropam de monstro)
   ============================================================ */
...
(function () {
  ...
  // ---------- Constantes ----------
  const LS_KEY = "pxg-stylist-craft-v1";
  const DEFAULT_SEM_DROPPERS = [
    "Style Point",
    "Refined Style Point",
    "Fashion Point",
    "Refined Fashion Point",
  ];
  ...
  function recomputeDepsAndRequirements() {
    ...
    // requiredByDep armazena EXECUÇÕES de receita (não unidades).
    // Pra unidades produzidas, basta multiplicar por child.unidadesPorReceita.
    let safety = 50000;
    while (queue.length && safety-- > 0) {
      ...
      for (const r of parent.recursos) {
        const childId = craftIdByName.get(normalizeName(r.nome));
        if (!childId) continue;
        const unitsNeeded = r.quantidade * parentQty;
        const child = craftsById[childId];
        const childUnits = child && child.unidadesPorReceita > 0 ? child.unidadesPorReceita : 1;
        const childExecutions = childUnits > 1 ? Math.ceil(unitsNeeded / childUnits) : unitsNeeded;
        requiredByDep.set(childId, (requiredByDep.get(childId) || 0) + childExecutions);
        currentDepsSet.add(childId);
        queue.push({ id: childId, qty: childExecutions });
      }
    }
    if (safety <= 0) {
      console.warn("recomputeDepsAndRequirements: safety hit — possível ciclo em CRAFTS_STYLIST");
    }
  }
  ...
  function buildResourceBadges(r) {
    ...
    if (subCraftId && subCraftId !== r.id) {
      ...
      b.textContent = "↳ Receita do Estilista";
      b.title = "Esse recurso também é uma receita do Estilista — o custo dele pode ser calculado a partir dos ingredientes dele";
      badges.push(b);
    }
    if (lootItem) {
  ...
  function recalcAll() {
    for (const craft of CRAFTS_STYLIST) recalcCraft(craft.id);
  }
  ...
  // ---------- Inicialização ----------
  async function init() {
    if (typeof CRAFTS_STYLIST === "undefined" || !Array.isArray(CRAFTS_STYLIST)) {
      console.error("CRAFTS_STYLIST não está disponível. Verifique stylist-data.js.");
      return;
    }

    // Pré-marca recursos da mecânica Drawing Book como matéria-prima
    // (Style Point / Fashion Point e variantes Refined). Só executa uma vez
    // por usuário — flag bootstrapDone fica no LS local desta página.
    try {
      const raw = localStorage.getItem(LS_KEY);
      const parsed = raw ? JSON.parse(raw) : {};
      if (!parsed.bootstrapDone) {
        for (const name of DEFAULT_SEM_DROPPERS) {
          if (!PxgShared.isSemDroppers(name)) PxgShared.setSemDroppers(name, true);
        }
        parsed.bootstrapDone = true;
        localStorage.setItem(LS_KEY, JSON.stringify(parsed));
      }
    } catch (e) {
      console.warn("Stylist bootstrap failed:", e);
    }

    // Aplica correções de nomes da wiki antes de qualquer indexação
    if (typeof WikiOverrides !== "undefined") {
      await WikiOverrides.load();
      WikiOverrides.applyToCrafts(CRAFTS_STYLIST);
    ...
    for (const c of CRAFTS_STYLIST) {
      craftsById[c.id] = c;
      craftIdByName.set(normalizeName(c.nome), c.id);
    }
    Object.assign(resourceIndex, buildResourceIndex(CRAFTS_STYLIST));
    ...
    document.getElementById("craftCount").textContent =
      `${CRAFTS_STYLIST.length} crafts`;

    const resources = getUniqueResources(CRAFTS_STYLIST);
    renderResourceInputs(resources);

    const tipoSelect = document.getElementById("filterTipo");
    for (const t of getUniqueTipos(CRAFTS_STYLIST)) {
    ...
    renderCraftCards(CRAFTS_STYLIST);
